...
def test_edit_own_message_as_member(user_channel_message_init):
    USERS, CHANDETAILS, MESSAGES = user_channel_message_init

    requests.put(BASE_URL + "/message/edit", json={
        'token': USERS[0]['token'],
        'message_id': 1,
        'message': "New edited messsage."
    })

    MESSAGES = requests.get(f"{BASEURL}{PORT}/messages/all").json()
    assert MESSAGES[0]['message'] == "New edited messsage."
...
def test_edit_deleted_message(user_channel_message_init):
    USERS, CHANDETAILS, MESSAGES = user_channel_message_init
...
def test_edit_member_message_as_owner(user_channel_message_init):
    USERS, CHANDETAILS, MESSAGES = user_channel_message_init
    ...
    MESSAGES = requests.get(f"{BASEURL}{PORT}/messages/all").json()
    assert MESSAGES[2]['message'] == "New edited messsage."

def test_edit_other_message_as_member(user_channel_message_init):
    USERS, CHANDETAILS, MESSAGES = user_channel_message_init
...
def test_edit_message_but_owner_not_in_channel(user_channel_message_init):
    USERS, CHANDETAILS, MESSAGES = user_channel_message_init
...
def test_message_edit_over_1000_chars(user_channel_message_init):
    USERS, CHANDETAILS, MESSAGES = user_channel_message_init
    message = 'a' * 1001
    with pytest.raises(requests.exceptions.HTTPError):
        requests.put(BASE_URL + "/message/edit", json={
...
def test_message_edit_to_nothing(user_channel_message_init):
    USERS, CHANDETAILS, MESSAGES = user_channel_message_init
