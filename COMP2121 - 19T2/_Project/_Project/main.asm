...
.dseg
array: .byte 100 ;10x10 array for storing names
times: .byte 11 ;11x1 array for storing station to station times
...
RESET:
	...
	;PWM and Motor Set Up
		...
		sts OCR3BL, temp ;Load pulse width/speed into OC4C low register
		clr temp
		sts OCR3BH, temp ;Load pulse width/speed into 0C4C high register
...
main:
	...
	call nextSection
	loop:
	call readPrint ;internal eternal loop
	jmp loop
...
;//=== Convertion Numbers Start ===\\
	...
	convertNumbers:
			...
			breq notNumberReturn ;but no letters so keypad check
			...
			cpi temp3, 0 ;if the current number is zero clr r9
			brne convertNumbers_cont 
			clr r9

			convertNumbers_cont:
			ldi mask, 2
			cp r9, mask ;Stop Calculating after the second number
			breq notNumberReturn
			...
			ldi temp2, AsciiNum
			add temp, temp2
			jmp convertNumbers_end

		notNumberReturn:
			ldi temp, 1
			ret ;return with value 1 in temp to show not a valid key
		...
		star: ;either store temp3 or clear it and err message
			...
			brsh convertNumbers_error
			cpi temp3, 0 ;is zero is also an err
			breq convertNumbers_error
			inc temp4
			rcall delay_30ms
			do_lcd_command 0b1
			clr temp
			ret ;return with value 1 in temp to show that entry has not concluded

		zero:
			cpi temp3, 0 ;If current value is zero and input is zero
			breq notNumberReturn ;Don't bother doing anything
			ldi mask, 2
			cp r9, mask
			breq notNumberReturn
			ldi temp2, 10
		...
		convertNumbers_end:
			inc r9 ;number of numbers entered counter
		...
		convertNumbers_error:
			ldi temp, 2
			ret ;return with value 2 in temp to show that the number was greater than 10
...
;//=== Convertion Characters Start ===\\
	...
	character_convert:
		...
		cpi col, LAST_COLUMN
		breq store_position_selection_relay
		...
		white_space:
			ldi temp, 0
			cp r15, temp ;only decrement if there is already characters in there
			brne normal_space
...
;//=== Stations Names ===\\
	...
	readInStations:
		...
		readInStations_start:
			clr r15
			cp temp4, stationNum
			breq readInStations_end
			clr temp5
			station_name_prompt temp4
			jmp readInStations_loop
			...
			readInStations_loop:
				...
				cpi temp5, '*'
				brne readInStations_loop

				ldi temp , 1
				cp r15, temp
				brlt readInStations_error

				inc temp4
				rcall delay_100ms
				jmp readInStations_start
		...
		readInStations_error:
			...
			readInStations_error_loop:
				...
				st z+, temp
				dec temp5
				brne readInStations_error_loop
			sbiw z, 10
			do_lcd_command 0b1
			do_lcd_data 'E'
			do_lcd_data 'r'
...
;//=== Time Between Stations Start ===\\
	...
	timeBetweenStations:
		do_lcd_command 0b1
		ld temp4, y+ ;leave the first value blank so the list of names and times match up
		clr temp4
...
;//=== Read and Print Stations Start ===\\
	...
	readPrint_loop:
		...
		brlo readPrint_normal ;not last name
		
		ldi zl, low(array) ;reset it to the start of name array
		ldi zh, high(array)
		ldi yl, low(times)
		ldi yh, high(times)
		clr currStation
		...
		travelling
		ldi temp, 0xFF
		cp temp, r5 ;check if button interrupt was triggered
		brne readPrint_cont_relay
		jmp readPrint_stopping
		readPrint_cont_relay: jmp readPrint_cont
		readPrint_stopping:
		stopping
